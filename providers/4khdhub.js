 ...
 * ========================================
 * Author: Xyr0nX & Nuvio Team
 * Enhanced Multi-Stream, FSL Domains, Smart Deduplication & Host Priority
 */
...
function cleanTech(title) {
  ...
  for (i = 0; i < parts.length; i += 1) {
    ...
    if (!part) continue;
    if ((allowed[part] || /^DDP\d\.\d$/.test(part)) && !seen[part]) {
      seen[part] = 1;
      out.push(part);
    }
  }
...
function resolveHubdrive(url, label, quality, meta) {
  ...
  return fetchText(url, { headers: { Referer: url } }).then(function(html) {
    ...
    $("a[href]").each(function(_, el) {
      ...
      if (!href) return;
      var lowerCandidate = href.toLowerCase();

      if (
        lowerCandidate.indexOf("drive.google") !== -1 ||
        lowerCandidate.indexOf("googleusercontent") !== -1 ||
        lowerCandidate.indexOf("hubcloud") !== -1 ||
        lowerCandidate.indexOf("workers.dev") !== -1 ||
        lowerCandidate.indexOf(".r2.dev") !== -1 ||
        lowerCandidate.indexOf("/download") !== -1 ||
        /\.(mkv|mp4|m3u8)(\?|#|$)/i.test(lowerCandidate) ||
        text.indexOf("download") !== -1
      ) {
        if (
          lowerCandidate.indexOf("/login") !== -1 ||
          lowerCandidate.indexOf("/register") !== -1 ||
          lowerCandidate.indexOf("javascript") !== -1 ||
          href === url
        ) return;
    ...
    if (!candidates.length) {
      ...
      if (downloadBtn) {
        ...
        return resolveLink(downloadUrl, label, url, quality, "", meta);
      }
      return [];
    }
...
function findContentUrl(tmdbId, mediaType, fallbackTitle) {
  return getTmdbNames(tmdbId, mediaType).then(function(names) {
    var queryTitle = names.title || fallbackTitle;
    if (!queryTitle && !names.original) return null;

    var key = queryTitle + " " + names.year;
    if (KNOWN_URLS[key]) {
      dbg("[findContentUrl] Found in KNOWN_URLS:", KNOWN_URLS[key]);
      return KNOWN_URLS[key];
    }

    return searchContent(queryTitle, mediaType, names.year).then(function(found) {
      if (found) return found;
      if (names.original && names.original !== queryTitle) {
...
function getStreams(tmdbId, mediaType, season, episode, titleFallback) {
  return getTmdbNames(tmdbId, mediaType).then(function(tmdbData) {
    var title = tmdbData.title || titleFallback;
    var epPromise = (mediaType === "tv") 
      ? getTmdbEpisodeName(tmdbId, season, episode) 
      : Promise.resolve("");

    return epPromise.then(function(epTitle) {
      return findContentUrl(tmdbId, mediaType, title).then(function(contentUrl) {
